<?php 
    ...
    if(isset($_GET['product_id'])){
        ...
        $product_img3        = $result['product_image2'];
        $product_img2        = $result['product_image3'];
        $product_price       = $result['product_price'];
        $product_price_cut   = $result['product_price_cut'];
        $product_keywords    = $result['product_keywords'];
        $product_desc        = $result['product_description'];
        $product_img         = [$product_img1,$product_img2,$product_img3];

        $get_p_categroy = $con->prepare("SELECT * FROM product_categories where p_cat_id = $p_cat_id");
        $get_p_categroy->execute();
        $result = $get_p_categroy->fetch(PDO::FETCH_ASSOC);
...
<html>
                ...
                <div class="col-md-6">
                    <ul class="menu">
                        ...
                        <li>
                            <?php
                            if(isset($_SESSION['user_name'])){
                                    echo "<a href='Users/my-account.php?my_orders'><i class='fa fa-user' aria-hidden='true'></i> My Account</a>";
                                }
                                else{
    ...
    <div id="navbar" class="navbar navbar-default navbar-box">
        ...
        <div class="navbar-header row">
            <a href="index.php" class="navbar-brand col-sm-4 col-md-2 col-lg-2">
            ...
            <div class="navbar-collapse collapse d-md-block col-md-10 col-lg-10 navigation" id="navigation">
                <div class="padding-nav float-left">
                    <ul class="nav left float-left">
                        ...
                        <li class="<?php if($active == 'MY ACCOUNT') echo 'active';?>">
                            <?php
                                if(isset($_SESSION['user_name'])){
                                    echo "<a href='Users/my-account.php?my_orders'>MY ACCOUNT</a>";
                                }
                                else{
                ...
                <?php 
                    if(!isset($_GET['product_id'])){
                        echo "
                            <a class='navbar-collapse collapse float-right d-md-block search-box' type='button' data-toggle='collapse' data-target='#search'>
                                <span class='sr-only'>Toggle Search</span>
                                <i class='fa fa-search'></i>
                            </a>
                        ";
            ...
            <div class="collapse search-box" id="search">
                    <form method="GET" action="" enctype="multipart/form-data" class="navbar-form" onsubmit="return searchKeywords();">
