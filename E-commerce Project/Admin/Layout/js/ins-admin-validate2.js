...
function checkInsert() {
    var admin_name = document.getElementById('admin_name'),
        ...
        arr_input = [admin_name, first_name, last_name, email, birthday, country, city, password, re_password],
    ...
    if (admin_name.value.length != 0 && first_name.value.length != 0 && last_name.value.length != 0 && email.value.length != 0 && birthday.value.length != 0 && country.value.length != 0 && city.value.length != 0 && password.value.length != 0 && re_password.value.length != 0) {
        if (check[6].classList.contains("checked") == true) {
            if (password.value === re_password.value) {
                $.ajax({
                    type: 'POST',
                    url: 'Includes/Components/ins-admin-check.php',
                    data: {
                    ...
                    success: function (response) {
                        ...
                        else if(response.search("admin name") > -1){
                            admin_name.focus();
                            notify[0].classList.add("invalid");
                            notify[0].textContent = "Admin name has already taken, try another one";
